...
class Table {
    ...
    private class Node {
        ...
        public Node next; // A pointer to the next node (or null)


        /********************************************************************
         * Constructor Node --- Constructs a leaf (no children)
         ********************************************************************/
        public Node( VariableRecord v, Node next ) {
            var = v;
            this.next = next;
        }
    ...
    public VariableRecord search ( String varID ) {
        ...
        for (Node a : hashTable) {
            ...
            while (curr != null && !curr.var.getIdentifier().equals(varID)) {
                curr = curr.next;
            } // end while

            // Return (name,value) pair if found, null if not.
            if (curr != null)
                result = curr.var;
        }
        return result;
